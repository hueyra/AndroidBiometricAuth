 ...
 * Desc : 授权触控ID页面
 */
class AuthTouchIDActivity : AppCompatActivity(), BiometricAuthCallback, OnAuthActionListener {
    ...
    private fun setUserInfo() {
        ...
        mAfaIvAuthImg.setImageResource(R.mipmap.ic_touch_id_green)
        mAfaIvAuthText.text = ("点击授权使用指纹识别")
        //设置Listener
    ...
    private fun startAuth() {
        ...
        if (mCurrentAuthTime >= 5) {
            ...
            if (check.isSuccess) {
                ...
                val soterCheck = BiometricAuth.getInstance().isSupportFingerprintAuth(this)
    ...
    override fun onShowAuthDialog(biometricType: Int) {
        if (!mAuthErrorDialog.isShowing()) {
            mSimpleAuthDialog.authWithTouchID()
        }
    }
    ...
    override fun onNotifyAuthDialogOnceAgain(biometricType: Int) {
        if (mCurrentAuthTime == 2) {
            ...
            BiometricAuth.getInstance().cancelAuth()
            mAuthErrorDialog.authErrorWithTouchID()
